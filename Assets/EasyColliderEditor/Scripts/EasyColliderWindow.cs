...
// merge multiple convex mesh colliders into a single static mesh collider.
// create colliders on a child.

// Improve API
// Continual code cleanup / refactoring


// can't reproduce enough to fix easily:
// occasionally cannot select vertices until scene camera rotates.
  ...
  public class EasyColliderWindow : EditorWindow
  {
    ...
    private void CreateCollider(CREATE_COLLIDER_TYPE collider_type, string undoString)
    {
      ...
      if (ECEPreferences.ColliderHolder != COLLIDER_HOLDER.Default)
      {
        ...
        {
          t = ECEditor.AttachToObject.transform.Find("Colliders");
    ...
    private void DrawPreferences()
    {
      ...
      if (_EditPreferences)
      {
        ...
        ECUI.FloatFieldUndoable(ECEPreferences, new GUIContent("Vertex Scale:", "Multiplier to all types of displayed points."), "Change common multiplier", ref ECEPreferences.CommonScalingMultiplier);
        // all the toggles.
        EditorGUILayout.BeginHorizontal();
