...
namespace ECE
{
  ...
  public class EasyColliderCreator
  {
...
#if (UNITY_EDITOR)
      List<Vector3> cylinderLocalPoints = CalculateCylinderPointsLocal(worldVertices, attachTo, ECEPreferences.CylinderNumberOfSides, ECEPreferences.CylinderOrientation, ECEPreferences.CylinderRotationOffset);
#else
      List<Vector3> cylinderLocalPoints = CalculateCylinderPointsLocal(worldVertices, attachTo, numberOfSides, orientation, cylinderOffset);
#endif
    ...
    public List<Vector3> CalculateCylinderPointsLocal(List<Vector3> vertices, Transform attachTo, int numberOfSides, CYLINDER_ORIENTATION orientation, float cylinderOffset)
    {
      ...
      {
        data = CalculateBox(vertices, attachTo);
      }
      ...
      // transform the box's center to world space if needed.
      Vector3 worldCenter = attachTo != null ? attachTo.TransformPoint(data.Center) : data.Center;
      ...
      {
        current.x = (direction == 0) ? worldCenter.x : v.x;
        current.y = (direction == 1) ? worldCenter.y : v.y;
        current.z = (direction == 2) ? worldCenter.z : v.z;
        distance = Vector3.Distance(current, worldCenter);
