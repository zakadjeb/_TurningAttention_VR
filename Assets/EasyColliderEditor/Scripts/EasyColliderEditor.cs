...
namespace ECE
{
  ...
  public class EasyColliderEditor : ScriptableObject, ISerializationCallbackReceiver
  {
    ...
    public void CreateCylinderCollider()
    {
      ...
      if (EasyColliderPreferences.Preferences.SaveConvexHullAsAsset)
      {
        EasyColliderSaving.CreateAndSaveMeshAsset(data.ConvexMesh, AttachToObject);
      }
    ...
    public void InvertSelection()
    {
      ...
      set.RemoveWhere(x => x.enabled == false);
      SelectedColliders = set.ToList();
    }


    // public Collider[] GetSelectableColliders()
    // {
    ...
    private void ToggleCollidersEnabled(bool colliderSelectionEnabled)
    {
      ...
      foreach (Collider col in colliders)
      {
        ...
        if (AddedInstanceIDs.Contains(col.GetInstanceID()))
        {
